...
export default class BuyerQuickCart extends Component {

    constructor(props) {
        super(props);
        const {params, data, ads, address, updateQuickCarter, handleDeleteQuickProduct} = this.props;
        this.state = {
            params: params,
            data: data,
            ads: ads,
    ...
    UNSAFE_componentWillMount() {
        const total = this.state.data.reduce(
            (pre, item) => {
                return pre + (item.checked ? (item.money * item.amount) : 0)
            }, 0);
    ...
    onChecked(id) {
        ...
        const newTotal = data.reduce((pre, item) => {
            return pre + (item.checked ? (item.money * item.amount) : 0)
        }, 0);
    ...
    deleteProduct = (id) => {
        ...
        this.setState({total: newTotal})
        this.state.handleDeleteQuickProduct(id)
    }
            ...
            <View style={styles.styledContainer}>
                ...
                <View style={styles.paymentContainer}>
                    <TouchableOpacity style={[styles.paymentButton, {backgroundColor: "#ed5829"}]} 
                      onPress={
                        () => {
                            const uploadedQuickCart = this.state.updateQuickCarter()
                            this.setState({data: uploadedQuickCart})
                        }
                      }
                    >
